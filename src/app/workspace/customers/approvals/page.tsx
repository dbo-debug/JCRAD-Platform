...
function ApprovalCard({ item }: { item: CustomerApprovalQueueItem }) {
  return (
    <article className="rounded-2xl border border-[#deded8] bg-white p-4 shadow-sm">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="space-y-2">
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#7891a0]">Approval Blocker</p>
            <h2 className="text-lg font-semibold text-[#181817]">{item.companyName}</h2>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link
            ...
            className="inline-flex rounded-full border border-[#cfdce4] px-4 py-2 text-sm font-semibold text-[#24404d] transition hover:border-[#1b1b1a] hover:text-[#1b1b1a]"
          >
            Open Account
          </Link>
        </div>
      ...
      <div className="mt-4 rounded-xl border border-[#e2edf2] bg-[#f7f7f4] px-3 py-3">
        ...
        {item.linkedDocuments.length > 0 ? (
          <div className="mt-2 grid gap-2 md:grid-cols-3">
            {item.linkedDocuments.map((doc) => {
              const content = (
                <>
                  <p className="text-sm font-semibold text-[#181817]">{doc.title}</p>
              ...
              return doc.href ? (
                <Link key={doc.id} href={doc.href} target="_blank" className="rounded-xl border border-[#deded8] bg-white px-3 py-2.5 transition hover:border-[#1b1b1a]">
                  {content}
                </Link>
              ) : (
                <div key={doc.id} className="rounded-xl border border-[#deded8] bg-white px-3 py-2.5">
                  {content}
                </div>
              );
            })}
  ...
  return (
    <div className="mx-auto w-full max-w-[1520px] space-y-6">
      ...
      <section className="space-y-3">
        ...
        {queue.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[#deded8] bg-[#f7f7f4] px-6 py-10 text-center text-sm text-[#5b7382]">
