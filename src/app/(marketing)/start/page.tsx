  ...
  return (
    <div className="mx-auto max-w-3xl">
      <section className="overflow-hidden rounded-[36px] border border-[#d8e6ed] bg-[linear-gradient(145deg,#ffffff_0%,#f2fbfb_48%,#f8fbff_100%)] shadow-[0_24px_60px_rgba(16,42,67,0.08)]">
        <div className="border-b border-[#e5d8ef] px-5 py-8 sm:px-8">
          <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#6f32b5]">JC RAD Start</p>
        ...
        <div className="grid gap-4 p-5 sm:p-8">
          {ACTIONS.map((action) => (
            <Link
              key={action.href}
              href={action.href}
              className="group rounded-[28px] border border-[#e5d8ef] bg-white p-5 transition hover:-translate-y-0.5 hover:border-[#b7d6de] hover:shadow-[0_18px_40px_rgba(16,42,67,0.08)]"
            >
              ...
              <p className="mt-4 text-sm font-semibold text-[#6f32b5] transition group-hover:text-[#0b5d56]">Open {action.title}</p>
            </Link>
          ))}
        </div>
      </section>
