    ...
    <Link
      ...
      className={[
        ...
        tone === "warn"
          ...
          : "border-[#eadff1] bg-white hover:border-[#8f52dc]",
      ].join(" ")}
    >
      ...
      <p className="mt-3 text-xs font-semibold uppercase tracking-[0.08em] text-[#6f32b5]">{ctaLabel}</p>
    </Link>
  );
}
...
export function QueueActionRow({ item }: { item: ActionRowItem }) {
  return (
    <Link
      href={item.href}
      className="flex items-start justify-between gap-3 rounded-lg border border-[#eadff1] bg-white px-3 py-3 text-sm transition hover:border-[#8f52dc] hover:bg-[#fdf7fb]"
    >
      <div className="min-w-0">
        <p className="font-semibold text-[#173543]">{item.title}</p>
        <p className="mt-1 text-xs text-[#5b7382]">{item.detail}</p>
        <p className="mt-2 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#6f32b5]">{item.ctaLabel}</p>
      </div>
      <span
        className={[
          "shrink-0 rounded-full px-2.5 py-0.5 text-xs font-semibold",
          item.tone === "warn" ? "bg-[#fff3dd] text-[#8a5a08]" : "bg-[#eef7f6] text-[#6f32b5]",
        ].join(" ")}
      >
        {item.count}
      </span>
...
export function SummaryBox({
  ...
  return (
    <div className="rounded-lg border border-[#eadff1] bg-white px-3 py-3">
    ...
    <div className="grid gap-2 sm:grid-cols-2">
      {cards.map((card) => (
        <Link
          key={card.href}
          href={card.href}
          className="rounded-lg border border-[#eadff1] bg-white px-3 py-3 transition hover:border-[#8f52dc] hover:bg-[#fdf7fb]"
        >
...
export function PlatformActivityList({ events }: { events: ActivityListItem[] }) {
  ...
  return (
    <div className="space-y-2">
      {events.map((row) => (
        <div key={row.id} className="rounded-lg border border-[#eadff1] bg-white px-3 py-2 text-sm">
...
export function DashboardPanel({
  ...
  return (
    <section className="rounded-xl border border-[#eadff1] bg-[#fdf8fd] p-4 shadow-sm">
      <div className="mb-3 flex items-start justify-between gap-3">
        ...
        {href && hrefLabel ? (
          <Link
            href={href}
            className="rounded-full border border-[#cfdce4] px-3 py-1 text-xs font-semibold text-[#2a4655] transition hover:border-[#8f52dc] hover:text-[#6f32b5]"
          >
            {hrefLabel}
          </Link>
        ) : null}
...
export function StatusPill({
  ...
  const toneClass =
    tone === "ok"
      ? "border-[#cde9e6] bg-[#eefaf8] text-[#6f32b5]"
      : tone === "bad"
