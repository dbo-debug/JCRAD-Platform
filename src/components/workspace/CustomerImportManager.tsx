...
export default function CustomerImportManager({ fields, canApply }: { fields: ImportField[]; canApply: boolean }) {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-2xl border border-[#deded8] bg-white p-6 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          {canApply ? (
            <button
              ...
              className="rounded-full border border-[#1b1b1a] px-4 py-2 text-sm font-semibold text-[#1b1b1a] disabled:opacity-60"
            >
        ...
        {successReport ? (
          <div className="mt-3 rounded-xl border border-[#b7efe6] bg-[#f2fffb] px-4 py-3 text-sm text-[#1b1b1a]">
      ...
      {preview ? (
        <>
          <section className="rounded-2xl border border-[#deded8] bg-white p-6 shadow-sm">
            <h2 className="text-lg font-semibold text-[#181817]">Column Mapping</h2>
          ...
          <section className="grid gap-4 md:grid-cols-5">
            {Object.entries(preview.summary).map(([key, value]) => (
              <div key={key} className="rounded-2xl border border-[#deded8] bg-white p-5 shadow-sm">
                <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#5d7685]">{key.replace(/_/g, " ")}</p>
                <p className="mt-2 text-2xl font-semibold text-[#181817]">{value}</p>
              </div>
            ))}
          </section>

          <section className="overflow-hidden rounded-2xl border border-[#deded8] bg-white shadow-sm">
              ...
              <tbody className="divide-y divide-[#eef3f6]">
                {preview.rows.map((row) => (
                  <tr key={row.rowNumber} className="align-top">
                    ...
                    <td className="px-4 py-4 text-[#181817] font-semibold">{row.companyName || "Missing Store Name"}</td>
