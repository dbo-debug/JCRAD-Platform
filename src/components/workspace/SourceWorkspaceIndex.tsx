...
function chipClass(value: string | null | undefined, kind: "status" | "stage" | "task" = "status") {
  ...
  if (kind === "task") {
    ...
    if (normalized === "open") return "border-[#d9ddd9] bg-[#f7f7f4] text-[#1b1b1a]";
...
function denseButtonClass(tone: "primary" | "secondary" = "secondary") {
  return tone === "primary"
    ? "inline-flex h-9 items-center justify-center rounded-full bg-[#181817] px-3.5 text-sm font-semibold text-white transition hover:bg-[#0f2a35]"
...
export default function SourceWorkspaceIndex({ sources, initialFilters }: SourceWorkspaceIndexProps) {
  ...
  return (
    <section className="space-y-4">
      <div className="rounded-[28px] border border-[#deded8] bg-white p-4 shadow-[0_16px_40px_rgba(15,42,53,0.08)]">
        ...
        <div className="mt-4 grid gap-3 xl:grid-cols-[minmax(0,1.3fr)_repeat(6,minmax(0,1fr))]">
          ...
          >
            <input
              ...
              className="h-9 w-full rounded-full border border-[#cedde6] bg-[#fafaf8] px-3.5 text-sm text-[#181817] outline-none transition focus:border-[#1b1b1a] focus:bg-white"
            />
      ...
      <div className="grid gap-4 xl:grid-cols-2 2xl:grid-cols-3">
        {filteredSources.map((source) => {
          ...
          return (
            <article
              key={source.id}
              className="group rounded-[28px] border border-[#deded8] bg-white p-5 shadow-[0_14px_35px_rgba(16,42,67,0.06)] transition hover:-translate-y-0.5 hover:border-[#b7d6e3] hover:shadow-[0_20px_48px_rgba(16,42,67,0.1)]"
            >
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <Link href={`/workspace/sources/${source.id}`} className="text-lg font-semibold text-[#181817] transition hover:text-[#1b1b1a]">
                    {source.name}
                  </Link>
                ...
                <div className="flex flex-col items-end gap-2">
                  <Link
                    ...
                    className="inline-flex h-10 items-center justify-center rounded-full bg-[#181817] px-4 text-sm font-semibold text-white transition hover:bg-[#0f2a35]"
                  >
                    Open Account
                  </Link>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <div className="rounded-2xl border border-[#e4eef3] bg-[#f7f7f4] p-3">
                  <p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#6c8796]">Primary Contact</p>
                  <p className="mt-1 text-sm font-medium text-[#181817]">{source.contactName || "Not set"}</p>
                  <p className="mt-1 text-sm text-[#4a6575]">
                    {emailHref ? (
                ...
                <div className="rounded-2xl border border-[#e4eef3] bg-[#f7f7f4] p-3">
                  ...
                  <p className="mt-1 text-sm font-medium text-[#181817]">{source.assignedBuyerName || "Unassigned"}</p>
              ...
              <div className="mt-4 flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full border border-[#deded8] bg-[#f7f7f4] px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-[#4f6877]">
              ...
              <div className="mt-5 flex flex-wrap items-center justify-between gap-3 border-t border-[#e7eff3] pt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {emailHref ? (
                    <a
                      href={emailHref}
                      className="inline-flex h-9 items-center justify-center rounded-full border border-[#deded8] bg-white px-3.5 text-sm font-medium text-[#42606f] transition hover:border-[#9eb6c4] hover:text-[#181817]"
                    >
                      Email
                    </a>
                  ) : null}
                  <Link
                    href={`/workspace/sources/${source.id}`}
                    className="inline-flex h-9 items-center justify-center rounded-full border border-[#1b1b1a] bg-[#f7f7f4] px-3.5 text-sm font-semibold text-[#1b1b1a] transition hover:bg-[#dff8f2]"
                  >
                    Open Account
                  </Link>
                </div>
      ...
      {filteredSources.length === 0 ? (
        <div className="rounded-[28px] border border-dashed border-[#deded8] bg-[#f7f7f4] px-6 py-10 text-center">
          <p className="text-lg font-semibold text-[#181817]">No sources match the current filters.</p>
